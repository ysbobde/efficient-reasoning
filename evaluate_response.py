...
parser.add_argument('--prompt_name', type=str, default="verifier2", help="prompt template to use")
...
def get_scores(ds, outputs, save_file_name=None):
    ...
    for input, output in zip(ds, outputs):
        # gold is wether the response is actually correct
        gold = str(input["accuracy"][0])  
        
        # what the verifier predicted
        prediction = [
    ...
    # SUGGESTION: we can add more metrics here like true/false positive/negative rates
    return {
        'pass@1': pass_at_1,
